...
{
    public class ObtenerRolHandler : IRequestHandler<RolVM, Iresult>
    {
        ...
        public async Task<Iresult> Handle(RolVM request, CancellationToken cancellationToken)
        {
            ...
            var position = _repositoryRol.TableNoTracking.Where(x => x.I_ID_ROL == request.I_ROLE_ID).ToList();

            var map = _mapper.Map<List<RolDTO>>(position);
