...
{
    public class AplicacionMenuHandler : IRequestHandler<AplicacionMenuVM, Iresult>
    {
        ...
        public async Task<Iresult> Handle(AplicacionMenuVM request, CancellationToken cancellationToken)
        {
            ...
            var menus = _repositoryMenu.TableNoTracking.Where(x => x.I_ID_APLICACION == request.I_APPLICATION_ID && x.B_ESTADO == "1").ToList();

            var submenus = _repositoryMenu.TableNoTracking.Where(x => x.I_ID_APLICACION == request.I_APPLICATION_ID && x.V_PARENTESCO != null && x.B_ESTADO == "1").ToList();
