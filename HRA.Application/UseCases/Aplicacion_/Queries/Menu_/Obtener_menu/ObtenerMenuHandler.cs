...
{
    public class ObtenerMenuHandler : IRequestHandler<MenuVM, Iresult>
    {
        ...
        public async Task<Iresult> Handle(MenuVM request, CancellationToken cancellationToken)
        {
            ...
            var menu = _repositoryMenu.TableNoTracking.Where(x => x.I_ID_MENU == request.I_MENU_ID).ToList();

            var map = _mapper.Map<List<MenuDTO>>(menu);
