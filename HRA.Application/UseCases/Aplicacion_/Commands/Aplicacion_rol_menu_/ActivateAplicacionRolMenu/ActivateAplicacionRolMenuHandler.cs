...
{
    public class ActivateAplicacionRolMenuHandler : IRequestHandler<ActivateAplicacionRolMenuVM, Iresult>
    {
        ...
        public ActivateAplicacionRolMenuHandler(
        ...
        public async Task<Iresult> Handle(ActivateAplicacionRolMenuVM request, CancellationToken cancellationToken)
        {
            ...
            var entity = _repositoryAppRolMenu.Table.FirstOrDefault(x => x.I_ID_APLICACION_ROL_MENU == request.I_APPLICATION_ROLE_MENU_ID && x.B_ESTADO == "0");
            ...
            if (entity == null)
            {
                return new FailureResult<IEnumerable<DetailError>>()
                {
                    Value = new List<DetailError>()
                    {
                        new DetailError("02", "No existe el registro o ya está activo")
                    }
                };
            }
