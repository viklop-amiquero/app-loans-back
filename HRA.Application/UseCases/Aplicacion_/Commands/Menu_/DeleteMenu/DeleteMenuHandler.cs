...
{
    public class DeleteMenuHandler : IRequestHandler<DeleteMenuVM, Iresult>
    {
        ...
        public async Task<Iresult> Handle(DeleteMenuVM request, CancellationToken cancellationToken)
        {
            ...
            if (entity == null)
            {
                return new FailureResult<IEnumerable<DetailError>>()
                {
                    Value = new List<DetailError>()
                    {
                        new DetailError("02","No existe el menú o ya está inactivo")
                    }
                };
