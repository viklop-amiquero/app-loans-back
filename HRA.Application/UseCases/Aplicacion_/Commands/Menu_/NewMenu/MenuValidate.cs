...
{
    public class MenuValidate : AbstractValidator<NewMenuVM>
    {
        public MenuValidate()
        {
            RuleFor(v => v.I_APPLICATION_ID)
            ...
            RuleFor(v => v.V_NAME)
                ...
                .Length(1, 50).WithMessage("Ingrese un menú con un máximo de 50 caracteres.");

            RuleFor(v => v.V_DESCRIPTION)
                .Matches(@"^(?:[A-Za-zñÑáéíóúÁÉÍÓÚ]+(?: [A-Za-zñÑáéíóúÁÉÍÓÚ]+)*)?$").WithMessage("La descripción del menú no es válida (mas de un espacio entre palabras, espacios al inicio o al final de la data de entrada, caracteres especiales o números)")
