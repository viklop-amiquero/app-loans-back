...
{
    public class ActivateRolHandler : IRequestHandler<ActivateRolVM, Iresult>
    {
        ...
        public async Task<Iresult> Handle(ActivateRolVM request, CancellationToken cancellationToken)
        {
            ...
            var entity = _repositoryRol.Table.FirstOrDefault(x => x.I_ID_ROL == request.I_ID_ROLE && x.B_ESTADO == "0");
            ...
            if (entity == null)
            {
                return new FailureResult<IEnumerable<DetailError>>()
                {
                    Value = new List<DetailError>()
                    {
                        new DetailError("02","No existe el rol o ya está activo")
                    }
                };
            ...
            _repositoryAppRolMenu.Table.Where(x => x.I_ID_ROL == entity.I_ID_ROL && x.B_ESTADO == "0").ToList().ForEach(a =>
            {
                a.B_ESTADO = "1";
                a.I_USUARIO_MODIFICA = usuario.I_ID_USUARIO;
                a.D_FECHA_MODIFICA = _repositoryDate.Now;

                _repositoryUsuarioApp.Table.Where(x => x.I_ID_APLICACION_ROL_MENU == a.I_ID_APLICACION_ROL_MENU && x.B_ESTADO == "0").ToList().ForEach(u =>
                {
                    u.B_ESTADO = "1";
