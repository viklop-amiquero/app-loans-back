...
{
    public class ActivateUsuarioHandler : IRequestHandler<ActivateUsuarioVM, Iresult>
    {
        ...
        public async Task<Iresult> Handle(ActivateUsuarioVM request, CancellationToken cancellationToken)
        {
            ...
            var entity = _repositoryUsuario.Table.FirstOrDefault(x => x.I_ID_USUARIO == request.I_ID_USER && x.B_ESTADO == "0" || x.B_ESTADO == "2");
            ...
            if (entity == null)
            {
                return new FailureResult<IEnumerable<DetailError>>()
                {
                    Value = new List<DetailError>()
                    {
                        new DetailError("02","No existe el usuario o ya está activo")
                    }
                };
            ...
            var clave_user = _repositoryClave.Table.Where(x => x.I_ID_USUARIO == entity.I_ID_USUARIO && x.B_ESTADO == "0")
            ...
            _repositoryUserApp.Table.Where(x => x.I_ID_USUARIO == entity.I_ID_USUARIO && x.B_ESTADO == "0").ToList().ForEach(u =>
            {
                u.B_ESTADO = "1";
