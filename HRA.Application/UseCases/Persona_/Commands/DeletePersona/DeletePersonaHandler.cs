...
{
    public class DeletePersonaHandler : IRequestHandler<DeletePersonaVM, Iresult>
    {
        ...
        public async Task<Iresult> Handle(DeletePersonaVM request, CancellationToken cancellationToken)
        {
            ...
            var entity = _repositoryPersona.Table.FirstOrDefault(x => x.I_ID_PERSONA == request.I_PERSON_ID);
            ...
            if (entity == null)
            {
                return new FailureResult<IEnumerable<DetailError>>()
                {
                    Value = new List<DetailError>()
                    {
                        new DetailError("02","No existe la persona")
                    }
                };
