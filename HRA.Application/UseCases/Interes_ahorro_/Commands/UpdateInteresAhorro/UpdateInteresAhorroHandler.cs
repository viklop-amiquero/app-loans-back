...
{
    public class UpdateInteresAhorroHandler : IRequestHandler<UpdateInteresAhorroVM, Iresult>
    {
        ...
        public UpdateInteresAhorroHandler(
        ...
        public async Task<Iresult> Handle(UpdateInteresAhorroVM request, CancellationToken cancellationToken)
        {
            int edad = 0;
            ...
            var entity = _repositoryInteresAhorro.Table.FirstOrDefault(x => x.I_ID_INTERES_AHORRO == request.I_INTEREST_ID && x.B_ESTADO == "1");
            ...
            if (_repositoryInteresAhorro.TableNoTracking.Where(x => x.V_NOMBRE == request.V_NAME && x.I_ID_INTERES_AHORRO != request.I_INTEREST_ID ).ToList().Count == 0)
            {
