...
{
    public class ActivateInteresCreditoHandler : IRequestHandler<ActivateInteresCreditoVM, Iresult>
    {
        ...
        public ActivateInteresCreditoHandler(
        ...
        public async Task<Iresult> Handle(ActivateInteresCreditoVM request, CancellationToken cancellationToken)
        {
            ...
            var entity = _repositoryInteresCredito.Table.FirstOrDefault(x => x.I_ID_INTERES_CREDITO == request.I_INTEREST_ID && x.B_ESTADO == "0");
            ...
            if (entity == null)
            {
                return new FailureResult<IEnumerable<DetailError>>()
                {
                    Value = new List<DetailError>()
                    {
                        new DetailError("02","No existe el interés o está activo")
                    }
                };
            }
