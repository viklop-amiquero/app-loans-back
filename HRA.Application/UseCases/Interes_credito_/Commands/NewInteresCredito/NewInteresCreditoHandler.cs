...
namespace HRA.Application.UseCases.Interes_credito_.Commands.NewInteresCredito
{
    public class NewInteresCreditoHandler : IRequestHandler<NewInteresCreditoVM, Iresult>
    {
        ...
        public NewInteresCreditoHandler(
        ...
        public async Task<Iresult> Handle(NewInteresCreditoVM request, CancellationToken cancellationToken)
        {
            ...
            if (_repositoryInteresCredito.TableNoTracking.Where(x => x.V_NOMBRE == request.V_NAME).ToList().Count == 0)
            {

                _repositoryInteresCredito.Insert(new List<Interes_credito>
                {
                    new Interes_credito
                    {
